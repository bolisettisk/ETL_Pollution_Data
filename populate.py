...
def Initialise_SQL(host_name, database_name, username, password):
    ...
    try:
        SQL_Initialize = mariadb.connect(
            host=host_name,
            user=username,
            passwd=password,
            autocommit=False # Preventing autocommit to load the data in chunks
        )
        # Function to check connection status
        def MariaDB_Connection_Status(SQL_Initialize, Connection_var):
            try:
                SQL_Initialize.ping()
            except:
                return print("Connection to {} is unsuccessful".format(Connection_var))
            else:
                return print("Connection to {} is successful".format(Connection_var))
        MariaDB_Connection_Status(SQL_Initialize,"SQL server") # Printing the connection status to SQL server
    ...
    # Creating pollution-db2 database
        ...
        try:
            ...
            try:
                ...
                MariaDB_Connection_Status(SQL_Initialize,"pollution-db2 database") # Printing the connection status to pollution-db2 database
            except mariadb.Error as err:
...
def Execute_Query(Initialisation_key, Query):
    try:
...
try:
    ...
    Create_Site_Query = ["""DROP TABLE IF EXISTS `pollution-db2`.`Site`;""", """CREATE TABLE IF NOT EXISTS `pollution-db2`.`Site` (
        ...
        PRIMARY KEY (`SiteID`))
    ENGINE = InnoDB;"""]
    Execute_Query(Initialisation_key, Create_Site_Query) # Creating the Site table
except mariadb.Error as err:
...
try:
    ...
    Create_Reading_Query = ["""DROP TABLE IF EXISTS `pollution-db2`.`Reading`;""", """CREATE TABLE IF NOT EXISTS `pollution-db2`.`Reading` (
        ...
        ON UPDATE NO ACTION)
    ENGINE = InnoDB;"""]
    Execute_Query(Initialisation_key, Create_Reading_Query) # Creating the Reading table
except mariadb.Error as err:
...
try:
    ...
    with open(source_filename, "r") as csv_data:
        for row in csv.reader(csv_data, delimiter=delimiter):
            ...
            if row_count%Chunk_Size == 0:
                ...
                SQL_Cursor.close() # Cursor is closed and reopened after each commit to prevent memory overflow
                SQL_Cursor = Initialisation_key.cursor()
            row_count += 1
except mariadb.Error as err:
    ...
    Initialisation_key.commit() # Committing fianl update to the Reading table
    SQL_Cursor.close()
...
try:
    ...
    ENGINE = InnoDB;"""]
    Execute_Query(Initialisation_key, Create_Schema_Query) # Creating the Pollution-Schema table
except mariadb.Error as err:
